...
class SportsVenueRepository(BaseRepository):
    ...
    def find_available(self, venue_type: str = None, ec_id: str = None) -> List[Dict[str, Any]]:
...
class SportsBookingRepository(BaseRepository):
    ...
    def check_conflict(self, venue_id: int, booking_date: str, start_time: str, end_time: str, exclude_id: int = None) -> bool:
        conditions = ["venue_id=%s", "booking_date=%s", "status NOT IN ('cancelled')", "deleted=0",
                       "((start_time<%s AND end_time>%s) OR (start_time<%s AND end_time>%s) OR (start_time>=%s AND end_time<=%s))"]
        params = [venue_id, booking_date, end_time, start_time, end_time, start_time, start_time, end_time]
        if exclude_id:
