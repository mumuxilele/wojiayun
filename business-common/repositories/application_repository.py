...
class ApplicationRepository(BaseRepository):
    ...
    def find_by_user(self,
        ...
        # 查询总数
        count_sql = "SELECT COUNT(*) as cnt FROM " + self.TABLE_NAME + " WHERE " + where_clause
        count_result = self.db.get_one(count_sql, params.copy())
        total = count_result.get('cnt', 0) if count_result else 0

        # 查询数据
        data_sql = ("SELECT * FROM " + self.TABLE_NAME +
                    " WHERE " + where_clause +
    ...
    def find_by_application_no(self, application_no: str) -> Optional[Dict[str, Any]]:
        ...
        sql = ("SELECT * FROM " + self.TABLE_NAME +
               " WHERE (application_no = %s OR app_no = %s) AND deleted = 0 LIMIT 1")
    ...
    def get_user_stats(self, user_id: str) -> Dict[str, int]:
        ...
        sql = ("SELECT "
               ...
               "  COUNT(*) as total "
               "FROM " + self.TABLE_NAME + " WHERE user_id=%s AND deleted=0")
        result = self.db.get_one(sql, [user_id])

        if result:
            return {k: int(result.get(k, 0) or 0) for k in
                    ('pending', 'processing', 'completed', 'cancelled', 'total')}
    ...
    def cancel(self, app_id: int, user_id: str = None) -> bool:
        ...
        sql = ("UPDATE " + self.TABLE_NAME +
               " SET status='cancelled', updated_at=NOW() WHERE " + where_clause)
    ...
    def find_all_with_filters(self,
        ...
        if type_code:
            conditions.append("a.app_type = %s")
            params.append(type_code)
        ...
        # 查询总数
        count_sql = "SELECT COUNT(*) as cnt FROM " + self.TABLE_NAME + " a WHERE " + where_clause
        count_result = self.db.get_one(count_sql, params.copy())
        total = count_result.get('cnt', 0) if count_result else 0

        # 查询数据 - LEFT JOIN application_types
        data_sql = ("SELECT a.*, t.type_name, t.category "
