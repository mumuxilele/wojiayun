"""
Cafeteria 菜品实体
对应表: ts_cafeteria_dish
"""
from .base_model import BaseModel


class CafeteriaDish(BaseModel):
    ...
    def __init__(self, **kwargs):
        ...
        self.nutrition_tags = kwargs.get('nutrition_tags')
    ...
    def to_dict(self):
        return {
            ...
            'nutrition_tags': self.nutrition_tags,
...
class CafeteriaOrder(BaseModel):
    ...
    def __init__(self, **kwargs):
        ...
        self.order_type = kwargs.get('order_type', 'dine_in')
        self.table_no = kwargs.get('table_no')
        self.items = kwargs.get('items', [])
        self.total_amount = float(kwargs.get('total_amount', 0))
        self.actual_amount = float(kwargs.get('actual_amount', 0))
        self.discount_amount = float(kwargs.get('discount_amount', 0))
    ...
    def to_dict(self):
        return {
            ...
            'user_phone': self.user_phone, 'order_type': self.order_type,
            'table_no': self.table_no, 'items': self.items,
            'total_amount': self.total_amount, 'actual_amount': self.actual_amount,
            'discount_amount': self.discount_amount, 'status': self.status,
            'pay_status': self.pay_status, 'remark': self.remark,
