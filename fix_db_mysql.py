...
def add_column(table_name, column_def, description):
    ...
    return run_sql(sql, description)

def create_table(sql, description):
    """创建表（如果不存在）"""
    try:
        conn = get_connection()
        with conn.cursor() as cur:
...
def fix_favorites_table():
    ...
    add_column("business_favorites", 
               "target_name VARCHAR(200) DEFAULT '' COMMENT '收藏对象名称'",
               "添加 target_name 字段")
...
def fix_members_table():
    ...
    add_column("business_members",
               "member_level VARCHAR(20) DEFAULT 'bronze' COMMENT '会员等级'",
               "添加 member_level 字段")
